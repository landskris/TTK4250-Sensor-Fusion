...
# local
import ex3_ekf.dynamicmodels as dynmods
import ex3_ekf.measurmentmodels as measmods
from ex3_ekf.gaussparams import GaussParams, GaussParamList
...
class EKF:
    ...
    def innovation_mean(
        ...
        x = ekfstate.mean
        zbar = self.sensor_model.h(x, sensor_state=sensor_state)  # predicted measurement
    ...
    def update(self,
        ...
        # Covariance update
        I = np.eye(x.shape[0])  # Identitiy matrix
        P_upd = (I - W@H)@P  # the covariance update
    ...
    def step(self,
        ...
        ekfstate_upd = self.update(z, ekfstate_pred, sensor_state=sensor_state)  #todo check correct use ekfstate-pred here
        if get_prediction:
            return ekfstate_upd, ekfstate_pred
        return ekfstate_upd
    ...
    def NIS(self,
        ...
        v, S = self.innovation(z, ekfstate, sensor_state=sensor_state)
        NIS = v@la.inv(S)@v.T  # v* S^-1 * v^T
        return NIS

    @classmethod
    def NEES(cls,
             ekfstate: GaussParams,
             # The true state to comapre against
             x_true: np.ndarray,
             ) -> float:
    ...
    def loglikelihood(self,
        ...
        # Log of Normal distribution is quadratic function
        # TODO: log likelihood, Hint: log(N(v, S))) -> NIS, la.slogdet.
        quadr_repr = self.NIS(z, ekfstate, sensor_state=sensor_state)  # (x_est - x_g)^T P^-1 ( X_est - x_g)
          # Derivative of quadratic repr with regard to x is -> P^-1 * (P^-1)^T
        ll = None
    ...
    def estimate_sequence(
        ...
        # Make sure the sensor_state_list actually is a sequence todo remove seems stupid ..
        ...
        for k, (zk, Tsk, ssk) in enumerate(zip(Z, Ts_arr, sensor_state_seq)):
            ekf_state_upd, ekf_state_pred = self.step(zk, init_ekfstate, Tsk, sensor_state=ssk, get_prediction=True)
            # Save state predictions and estimates
            ekfupd_list[k] = ekf_state_upd
            ekfpred_list[k] = ekf_state_pred

            # Update state by update
            init_ekfstate = ekf_state_upd
