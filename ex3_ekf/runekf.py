# %% Imports
from ex3_ekf import dynamicmodels, measurmentmodels, ekf
...
if usePregen:
    ...
    from ex3_ekf.sample_CT_trajectory import sample_CT_trajectory
...
x_bar_init = np.array([1, 1, 0, 0])
P_bar_init = np.diag([sigma_a, sigma_a, sigma_a, sigma_a])**2  # TODO: ArrayLike with 4 x 4 elements, hint: np.diag
init_ekfstate = ekf.GaussParams(x_bar_init, P_bar_init)

# estimate - without starting with prediction, wait for first measurement
ekfpred_list, ekfupd_list = ekf_filter.estimate_sequence(Z=Z, init_ekfstate=init_ekfstate, Ts=Ts,
                                                         start_with_prediction=False)
